...
public class Student {
    ...
    public static boolean verifyYear(String year) {
        ...
        if (yearvalue < 1900 || yearvalue > 2021) {
            loggerStud.log(Level.SEVERE, "Error birthday! ");
            throw new IllegalArgumentException("Date of birth impossible!");
        }
        return true;
    }
    ...
    public static boolean verifyEmpty(@NotNull String name) throws Validation {
        if (name.equals("") || name.equals(" ")) {
            loggerStud.log(Level.SEVERE, "error, is empty");
            throw new Validation("Name must be entered!");
        }
        return true;
    }
    ...
    public static boolean verifyUpperLower(String genders) {
        if ((!genders.equalsIgnoreCase("m") && !genders.equalsIgnoreCase("F"))) {
            loggerStud.log(Level.SEVERE, "error, is not an gender");
            throw new IllegalArgumentException("It's not a gender!");
        }
        return true;
    }
